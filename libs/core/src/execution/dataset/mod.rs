...
pub trait DataSetExt {
    fn hash_group_by_exprs(&self, exprs: &mut [PhysicalExpr]) -> Result<HashGroupByExprsIter>;
...
impl DataSetExt for DataSet {
    fn hash_group_by_exprs(&self, exprs: &mut [PhysicalExpr]) -> Result<HashGroupByExprsIter> {
        hash_group_by_exprs(self, exprs)
    }

    fn group_by_exprs(&self, exprs: &mut [PhysicalExpr]) -> Result<GroupByExprsIter> {
        group_by_exprs(self, exprs)
    }

    fn group_by_window(&self, time_idx: usize, window: &Window) -> Result<GroupByWindowIter> {
        group_by_window(self, time_idx, window)
    }
}
...
fn create_record_keys(dataset: &DataSet, exprs: &mut [PhysicalExpr]) -> Result<Vec<Key>> {
...
pub type HashGroupByExprsIter<'a> = Box<dyn Iterator<Item = Result<(u64, DataSet)>> + 'a>;

fn hash_group_by_exprs<'a>(
    dataset: &'a DataSet,
    exprs: &mut [PhysicalExpr],
) -> Result<HashGroupByExprsIter<'a>> {
    let mut record_keys = create_record_keys(dataset, exprs)?;
    let num_group_exprs = exprs.len();
    ...
    for row in 0..dataset.len() {
        ...
        grouped_key.hash(&mut hasher);
        keys_map.entry(hasher.finish()).or_default().push(row);
    }

    Ok(Box::new(keys_map.into_iter().map(
        move |(hash, indexes)| create_dataset(dataset, &indexes).map(|dataset| (hash, dataset)),
    )))
}
